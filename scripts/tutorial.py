...
def example_1_process_url():
    ...
    try:
        reports = pipeline.execute(source=url, resource_id='example-resource-123', is_url=True)
...
def example_3_process_folder():
    ...
    for file_path in files:
        ...
        try:
            reports = pipeline.execute(source=str(file_path), resource_id=f'batch-{file_path.stem}', is_url=False)
...
def example_4_custom_configuration():
    ...
    try:
        reports = pipeline.execute(source=file_path, resource_id='custom-isp-123', is_url=False, isp_rules=isp_rules)
...
def print_results(reports: List[SheetReport]):
    ...
    for report in reports:
        ...
        print(
            f'   Models: PII={report.pii_classifier_model}, '
