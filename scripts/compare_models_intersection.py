...
def get_files_in_dir(directory):
    return {os.path.basename(f) for f in glob.glob(os.path.join(directory, '*.json')) if not os.path.basename(f).endswith('metrics_summary.json') and not os.path.basename(f).endswith('scores.json')}
...
def main():
    ...
    # Metrics storage
    results = {name: {'tp_p': 0, 'fp_p': 0, 'fn_p': 0, 'tn_p': 0, 'tp_np': 0, 'fp_np': 0, 'fn_np': 0, 'tn_np': 0} for name in dirs if 'groundtruth' not in name}
    ...
    # Print Report
    print(f"{'Model':<20} | {'Type':<15} | {'Prec':<6} | {'Recall':<6} | {'F1':<6} | {'Acc':<6} | {'TP':<3} | {'FP':<3} | {'FN':<3} | {'TN':<3}")
    print("-" * 100)
    ...
    for name, metrics in results.items():
        for label_type in ['p', 'np']:
            ...
            print(f"{name:<20} | {type_str:<15} | {precision:<6.2f} | {recall:<6.2f} | {f1:<6.2f} | {accuracy:<6.2f} | {tp:<3} | {fp:<3} | {fn:<3} | {tn:<3}")
