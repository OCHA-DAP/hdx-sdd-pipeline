...
class AzureOpenAIProvider(ILLMProvider):
    ...
    def generate(self, prompt: str, max_tokens: int = 256, temperature: float = 0.0, **kwargs) -> Tuple[str, int, int]:
        ...
        try:
            ...
            logger.debug(f"Generated {completion_tokens} completion tokens, " f"{prompt_tokens} prompt tokens")
    ...
    def generate_json(
        ...
        try:
            ...
            logger.debug(
                f"Generated JSON with {completion_tokens} completion tokens, " f"{prompt_tokens} prompt tokens"
            )
            ...
            logger.error(f"Azure OpenAI JSON generation failed: {e}")
