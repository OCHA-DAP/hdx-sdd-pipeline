    ...
    <div class="section">
      ...
      <table>
        ...
        <tbody>
          ${Object.entries(statistics.models)
            .map(([modelName, stats]: [string, any]) => {
              const bestF1 = Math.max(...Object.values(statistics.models).map((s: any) => s.file_level_non_pii?.f1 || 0));
              const isBest = (stats.file_level_non_pii?.f1 || 0) === bestF1;
              return `
                <tr>
                  <td><strong>${modelName}</strong>${isBest ? '<span class="best-badge">Best</span>' : ''}</td>
                  <td class="text-center metric-value">${formatPercent(stats.file_level_non_pii?.accuracy || 0)}</td>
                  <td class="text-center metric-value">${formatPercent(stats.file_level_non_pii?.precision || 0)}</td>
                  <td class="text-center metric-value">${formatPercent(stats.file_level_non_pii?.recall || 0)}</td>
                  <td class="text-center metric-value">${(stats.file_level_non_pii?.f1 || 0).toFixed(3)}</td>
                  <td class="text-center">${stats.file_level_non_pii?.total_files || 0}</td>
                </tr>
              `;
            }).join('')}
        </tbody>
      ...
      <table>
        ...
        <tbody>
          ${Object.entries(statistics.models)
            .map(([modelName, stats]: [string, any]) => {
              const bestF1 = Math.max(...Object.values(statistics.models).map((s: any) => s.sheet_level_non_pii.f1));
              const isBest = stats.sheet_level_non_pii.f1 === bestF1;
              return `
                <tr>
                  <td><strong>${modelName}</strong>${isBest ? '<span class="best-badge">Best</span>' : ''}</td>
                  <td class="text-center metric-value">${formatPercent(stats.sheet_level_non_pii.accuracy)}</td>
                  <td class="text-center metric-value">${formatPercent(stats.sheet_level_non_pii.precision)}</td>
                  <td class="text-center metric-value">${formatPercent(stats.sheet_level_non_pii.recall)}</td>
                  <td class="text-center metric-value">${stats.sheet_level_non_pii.f1.toFixed(3)}</td>
                  <td class="text-center">${stats.sheet_level_non_pii.total_sheets || 0}</td>
                </tr>
              `;
            }).join('')}
        </tbody>
        ...
        <div className="space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <ConfusionMatrixDisplay cm={modelStats.sheet_level_non_pii.confusion_matrix} title="Non-Personal Sensitive Data Confusion Matrix" />
            </div>
          </div>
      ...
      {activeView === 'file' && (
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              <MetricCard title="Accuracy" value={formatPercent(modelStats.file_level_non_pii?.accuracy || 0)} />
              <MetricCard title="Precision" value={formatPercent(modelStats.file_level_non_pii?.precision || 0)} />
              <MetricCard title="Recall" value={formatPercent(modelStats.file_level_non_pii?.recall || 0)} />
              <MetricCard title="F1 Score" value={formatNumber(modelStats.file_level_non_pii?.f1 || 0)} />
            </div>
            {modelStats.file_level_non_pii?.confusion_matrix && (
              <ConfusionMatrixDisplay 
                cm={modelStats.file_level_non_pii.confusion_matrix} 
                title="Non Personal Sensitive Data Detection Confusion Matrix" 
              />
            )}

            {/* Non-Personal Sensitive Data Misclassifications */}
            {modelStats.file_level_non_pii?.misclassifications && modelStats.file_level_non_pii.misclassifications.length > 0 && (
              <div className="mt-4">
                <h4 className="text-lg font-semibold text-white mb-3">
                  Non-Personal Sensitive Data Misclassifications ({modelStats.file_level_non_pii.misclassifications.length})
                </h4>
                <div className="border border-white/20 rounded-lg overflow-hidden bg-white/5">
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {modelStats.file_level_non_pii.misclassifications.map((error, idx) => (
                          <tr
                            key={idx}
                            className={`border-t border-white/10 ${
                              error.error_type === 'False Negative'
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {statistics.available_models.map((model, idx) => {
                      const stats = statistics.models[model];
                      if (stats.file_level_non_pii?.error) return null;
                      
                      const isTopF1 = statistics.available_models
                        .filter(m => !statistics.models[m].file_level_non_pii?.error)
                        .every(m => (statistics.models[m].file_level_non_pii?.f1 || 0) <= (stats.file_level_non_pii?.f1 || 0));
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="text-center py-3 px-3 text-white">
                            {formatPercent(stats.file_level_non_pii?.accuracy || 0)}
                          </td>
                          <td className="text-center py-3 px-3 text-white">
                            {formatPercent(stats.file_level_non_pii?.precision || 0)}
                          </td>
                          <td className="text-center py-3 px-3 text-white">
                            {formatPercent(stats.file_level_non_pii?.recall || 0)}
                          </td>
                          <td className="text-center py-3 px-3 text-white font-semibold">
                            {(stats.file_level_non_pii?.f1 || 0).toFixed(3)}
                          </td>
                          <td className="text-center py-3 px-3 text-white/70">
                            {stats.file_level_non_pii?.total_files || 0}
                          </td>
                        </tr>
                      );
                    })}
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {statistics.available_models.map((model, idx) => {
                      const stats = statistics.models[model];
                      if (stats.sheet_level_non_pii.error) return null;
                      
                      const isTopF1 = statistics.available_models
                        .filter(m => !statistics.models[m].sheet_level_non_pii.error)
                        .every(m => statistics.models[m].sheet_level_non_pii.f1 <= stats.sheet_level_non_pii.f1);
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="text-center py-3 px-3 text-white">
                            {formatPercent(stats.sheet_level_non_pii.precision)}
                          </td>
                          <td className="text-center py-3 px-3 text-white">
                            {formatPercent(stats.sheet_level_non_pii.recall)}
                          </td>
                          <td className="text-center py-3 px-3 text-white font-semibold">
                            {stats.sheet_level_non_pii.f1.toFixed(3)}
                          </td>
                          <td className="text-center py-3 px-3 text-white/70">
                            {stats.sheet_level_non_pii.total_sheets || 0}
                          </td>
                        </tr>
                      );
                    })}
