...
- [x] FR-SDD-057: GLiNER fast full-table PII pre-scan.
  - Expected behavior: Before the LLM-based PII classification step, the pipeline must run a fast, local GLiNER model scan over **all rows and all columns** of every data sheet to detect personal names, email addresses, and exact street addresses. If any are found (score ≥ configured threshold, default 0.5), the sheet is immediately flagged as `personal_data_sensitive=True` with `personal_data_classification.sensitivity=SEVERE_SENSITIVE`, column-level sensitive flags are set, and the LLM PII classification and reflection steps are **skipped** (reusing the existing early-exit pattern). Non-PII classification continues normally. The scan must:
    ...
    - Process rows in batches (default 256 rows) so that very large datasets are handled with bounded memory.
    ...
    - Expose `GLINER_THRESHOLD` (default `0.5`), `GLINER_MODEL`, and `GLINER_BATCH_SIZE` as environment-driven configuration settings.
